...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="bg-slate-900 text-white border-b border-slate-800 px-4 py-2.5 flex items-center justify-between gap-3 flex-wrap select-none z-20">
      ...
      <div className="flex items-center gap-2">
        {/* Cloud Resumes / Sync Button */}
        <button
          id="nav-cloud-resumes-btn"
          onClick={onOpenCloudResumesModal}
          ...
          title={user ? 'Manage & Sync your Cloud Resumes' : 'Sign in to save resumes in the cloud'}
        >
          <Cloud className="w-3.5 h-3.5 text-blue-400" />
          <span className="hidden sm:inline">
            {user ? 'My Resumes' : 'Cloud Sync'}
          </span>
          {activeCloudResumeId && user && (
        ...
        <div className="relative" ref={userDropdownRef}>
          {user ? (
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer"
            >
